...
# When running as app/app.py, __file__ is inside app/
# PROJECT_DIR = the repo root (one level up from app/)
...
# ── Auto-train models if missing (HF Spaces cold start) ─────────────────────

def _auto_train_if_needed():
    needed = ["model_real.pkl", "model_synthetic.pkl", "model_combined.pkl"]
    missing = [f for f in needed if not os.path.exists(os.path.join(MODELS_DIR, f))]
    if missing:
        print(f"[INFO] Missing models: {missing} — training now (first run only)...")
        import subprocess
        train_script = os.path.join(APP_DIR, "model", "train_model.py")
        ...
        if result.returncode == 0:
            ...
            print("[WARN] Training failed — app may have limited functionality.")
...
def predict_and_schedule(
    ...
    if PREDICT_MODEL is None:
        err = "⚠️ Models still loading — please refresh the page in 30 seconds."
        return err, err, None
